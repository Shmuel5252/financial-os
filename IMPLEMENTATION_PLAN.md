...
**Final acceptance (2026-09-07): ACCEPTED** at the documented real staged Financy boundary and ADR-066 development-only exception. The single newly authorized refresh completed through the actual authenticated UI/service/provider path, advanced the provider data date to 2026-09-07, and was followed by two successful ordinary syncs; repeat changes/duplicate groups are zero and BSON digests match. Current observations: 8 products/1,140 transactions; canonical provider records: 5 accounts/462 transactions. Full regression passed 74 files / 340 tests with real MongoDB, Financy, Anthropic and Resend test-mode gates; type-check, zero-warning lint/build, zero-vulnerability audit and authenticated Hebrew/RTL/security/integrity gates passed. See `PHASE_9_ACCEPTANCE_REPORT.md`. Commit/push/clean synchronization are the checkpoint handoff; after them the newest owner instruction permits the Phase 18 prerequisite review, not guessed production decisions. All older statuses immediately below are retained chronological evidence and are superseded by this final acceptance.

**Latest execution authority (2026-09-06, ADR-066):** Perform the owner's development-only replacement of exactly identified obsolete Financy ingestion, with verified recoverable archival and unchanged protected/manual data. The current ACTIVE connection is the new baseline. The earlier manual UI stop below is superseded only for this test dataset; preserve that workflow and production fail-closed reconciliation. Verify all current supported product/transaction observations, separate products, stable identity capture and repeat-sync idempotency, then original regression/provider/Mongo/browser/security gates. No paid refresh without approval; no Phase 18. This is not yet accepted.

**Current result:** Development archive/cutover, all 101 protected-document digests, 8-product/1,139-observation import, repeated-sync idempotency, actual authenticated sync and 74-file/335-test real-provider regression pass. Type-check, zero-warning lint/build and zero-vulnerability audit pass. The sole identified outstanding lifecycle gate is a real successful refresh receipt through the corrected adapter/service: the prior paid call returned provider `accepted` but left `failed/schema` in the application. A new 20-credit operation requires explicit owner approval; do not repeat disconnect. See `PHASE_9_ACCEPTANCE_REPORT.md`. No self-acceptance/commit/push or Phase 18 yet.

**Current handoff (2026-09-06, ADR-065): Manual UI ready; owner confirmations pending; Phase 9 unaccepted.** The owner-authorized manual comparison/attestation/rejection/uncertainty workflow is implemented. Canonical IDs and history remain unchanged; external identity claims and immutable events are attached through an atomic unique owner-scoped ledger. Future observations retain masked/HMAC stable identity evidence. Local regression passed 68 files / 319 tests with real MongoDB, plus the separate real Financy read-only comparison, type-check, zero-warning lint, production build, zero-vulnerability audit, and authenticated Hebrew/RTL read-only browser gate. Stop for explicit real owner confirmations in the UI. Afterward complete deterministic transaction reconciliation, original lifecycle/duplicate/idempotency/security/provider/browser acceptance, then and only then commit/push/accept. Older blocked checkpoint details below are retained as history. Phase 18 remains prohibited.

**Status (2026-09-06, latest): BLOCKED on historical identity reconciliation and unaccepted.** The unchanged private credentials now expose the new `ACTIVE` connection under the configured subject; the earlier access blocker is resolved. All current account/transaction IDs differ from retained canonical identities, and documented old-connection reads expose no bridging records. ADR-063 adds a tested owner-scoped pre-write rejection with a Hebrew reconciliation-required error, not a guessed merge. Authoritative identity evidence or an approved explicit reconciliation policy, remaining lifecycle evidence, final regression, and authenticated browser acceptance are mandatory before commit/push/self-acceptance. No post-reconnection import or paid refresh was performed. Phase 18 remains prohibited.

- **Objective:** Synchronize consented bank data through a licensed provider while preserving manual mode.
- **Latest verified gate (2026-09-06):** Actual provider account numbers are present (8/8) but absent from all historical observation identities (8/8), including five canonical accounts; parsed bank/branch/number are current-only (2/8). Five unique weak-label candidates do not prove continuity. Read-only real-provider/Mongo inventory passes, protected state is unchanged, and the guard still blocks. Per the approved insufficient-evidence rule, stop for authoritative bridging evidence or the explicit auditable manual reconciliation workflow; no account/transaction remap, final acceptance, commit/push, or Phase 18.
- **Approved reconnect policy (2026-09-06, ADR-064):** Provider aliases are not canonical identity. Preserve canonical IDs and append immutable old/new external provenance; require sufficient unique deterministic account mapping before transaction fingerprints. No amount/date-only deduplication, guessed tie-break, or unsupported pending/posted merge. Unresolved identity blocks canonical import and requires safe authenticated auditable manual review. First establish actual current/historical evidence sufficiency read-only; if insufficient, stop and identify missing fields and the manual gate. This resolves the policy question, not the data mapping or acceptance; retain the pre-write guard and all original gates.
...
- **Dependencies:** Selected licensed provider, hosted user consent, normalized Phase 2 domain, owner/provider-subject binding, server-only short-lived token cache. General multi-user hosted production onboarding remains a Phase 18/20 infrastructure gate.
- **Major tasks:** Provider-neutral port and Financy adapter, exact normalization, owner binding, idempotent leased pull sync, reconciliation UX, explicit cost-confirmed refresh, explicit disconnect/revoke, backoff/recovery. Do not invent a financial-data webhook without a documented provider security contract.
- **Data/model implications:** Owner-scoped bindings/connections, immutable observation revisions, sync runs, HMAC-aliased provider references, normalized source metadata; no bank username/password and no persisted access token.
- **Security:** Least-scope consent, in-memory token lifetime, origin/rate/body protections, credential/raw-payload redaction, owner-first indexes, private-by-default resources, explicit revocation/deletion.
...
- **Definition of done:** Real paid-provider staged run, repeat-sync reconciliation, lifecycle/failure checks, monitoring/runbook, real-Mongo isolation, and authenticated Hebrew/RTL acceptance verified. Refresh and deletion are exercised only after their separately confirmed external effects.
...
- **Approved product/ethics policy:** Progress is deterministic, explanatory, private, and supportive. It may recognize verified goal milestones (25/50/75/100), under-budget periods, no-overdraft periods, Safety Margin maintenance, scheduled debt-payment progress, and positive confirmed cash flow. It must never reward spending, investing, borrowing, debt creation, trading, unnecessary app checking, or risk-taking; it must never use rankings, social comparison, streak pressure, gambling language, shame, urgency, fear, confetti, or loss aversion.
- **Major tasks:** Add a versioned pure progress-policy engine, explicit authenticated evaluation/backfill (never page-load writes), append-only owner-scoped evidence with stable logical identity and supersession, current projections, timezone-aware healthy streaks, Goal Engine milestone consumption without recalculation, accessible Hebrew/RTL presentation, and independent controls for celebrations, streak display, and progress notifications.
- **Data/model implications:** `progressJourneyEvents` is immutable evidence with event/rule/policy versions, owner/scope, evaluation time and calendar period, trustworthy source references and fingerprint, live/backfill origin, outcome, and optional supersession. `progressJourneyPreferences` is optimistic and audited. Current state is a projection; corrections append evidence and never rewrite prior achievement. Missing evidence is `unknown`, not success or regression. No universal financial-health score and no duplicated canonical financial truth.
- **Security:** Personal/private by default. All actor identity is server-derived and every repository query is owner-first. Household evidence may be used only when every source is currently authorized; Phase 17 introduces no household sharing or comparison. AI remains downstream explanation only and cannot determine eligibility, status, streaks, milestones, regression, or financial truth.
- **Notifications:** Progress notifications require an independent explicit opt-in, obey Phase 15 quiet hours/deduplication/minimization, remain non-critical, and never bypass quiet hours. Disabling progress UI or notifications cannot suppress deterministic financial warnings or mutate any engine.
- **Testing:** Milestone identity and 25/50/75/100 consumption; streak start/continuation/break/recovery; regression and retained history; live/backfill/unknown; correction and policy-version supersession; timezone boundaries; risky-incentive exclusions; preference opt-outs with critical warnings retained; Goal/Safe-to-Spend/AI authority boundaries; owner/direct-ID/two-user/household privacy; notification quiet-hours/deduplication; Hebrew/RTL/accessibility; real MongoDB and regressions through Phase 16.
- **Acceptance criteria:** Every presented achievement derives from versioned authorized deterministic evidence, remains explainable after correction, exposes regression without shame, never pressures unsafe behavior, and honors independent disable controls without weakening critical product truth.
- **Definition of done:** The approved product/ethics policy is encoded and tested; authenticated Hebrew/RTL production acceptance passes; real MongoDB ownership/integrity and provider regressions pass; nonessential progress presentation can be disabled independently.
- **Risks/migration:** Harmful incentives and backfill changes after policy updates.
- **Verified result (2026-09-03):** Accepted under the execution-order exception. Dedicated Phase 17 coverage passed 4 files / 18 tests, including 5 real-Mongo integration tests; the final sequential suite passed 66 files / 299 tests without skips with real MongoDB, Anthropic, and Resend. Stable milestone identity, timezone-aware streaks, regression/recovery, stale/unknown handling, explicit trustworthy backfill, append-only correction/supersession, owner/two-user isolation, notification opt-in with critical-warning retention, risky-incentive exclusions, export schema 9, authenticated Hebrew/RTL production acceptance, type-check, zero-warning lint, optimized build, and a zero-vulnerability audit passed. Phase 9 remains blocked; Phase 18 cannot be fully accepted until the feature-complete/provider gate and production infrastructure exist.
