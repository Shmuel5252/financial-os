...
- **Evidence correction:** Owner reports deployed Vercel staging/Git main, Atlas persistence, full Google/Auth.js login after `812b280`, and deployed management QA. Record as owner-verified deployed evidence; repository verification and independently unverified external settings are separate. Earlier no-Vercel/loopback-only and pending-login statements are historical and superseded, not rewritten acceptance results. Refer to PHASE_18_ENTRY_REVIEW.md.
- **Approved directions:** Provisional SLO 99.5% successful eligible core requests over rolling 30 days; 10 concurrent synthetic staging users for 30 minutes; metadata/health/control-only explicit admin allowlist. Existing RPO <=24h, RTO <=4h, 30-day backup history, isolated restore and environment separation remain binding. Eligibility/alert/operator/budget details and actual external verification remain pending; no monthly SLO compliance is inferred.
- **Key boundary:** Authentication keys must eventually be separate from versioned Financy identity keys. Current AUTH_SECRET HMAC coupling is documented; no rotation or migration now. A future transition must preserve old aliases, evidence and deduplication, not reimport the bank dataset.
- **Privacy:** PHASE_18_DATA_INVENTORY.md covers 52 source collections and explicit embedded/cross-owner/archival consequences. Full-erasure versus immutable-audit treatment is NOT decided here. In particular, financial audit values/archives are not anonymous metadata; restore must not revive erased users, old sharing, sessions or external commands. No retention duration beyond approved backup history is invented.
- **Narrow redaction:** Installed Auth.js logger serializes upstream messages/stacks/cause data with debug false. Synthetic regression reproduced the exposure path before the fix. Override every logger level with fixed class-derived categories, random correlation ID and a redaction version; never raw names/messages/causes/URLs/codes/tokens/metadata. This removes a demonstrated repository leak path without claiming an actual historical credential leak. Errors/checks/sessions still behave normally; diagnostic detail is intentionally reduced. Platform access logs remain an external review gate.

## ADR-068 — Management UX reuses canonical workflows and cannot rewrite goal achievement

Release checkpoint: the owner subsequently authorized one commit/push of the already verified cleanup, not further implementation. The selected regression was rerun (361 passed; one explicitly skipped real reconnection test; unrelated external/destructive files excluded), alongside passing type-check, zero-warning lint, production build and a dependency audit with 0 vulnerabilities. Existing browser evidence is retained without claiming a new staging deployment test. Validate Vercel staging against the cleanup commit separately; no next-phase progression follows this release.

The owner's manual-QA cleanup request authorizes a focused presentation/functional correction, not a new goal-task domain or Phase 18 implementation. Inspection found duplicated page navigation, onboarding URLs usable after completion, summary-only record rows despite complete stored fields, and generic manual-goal labels. Share navigation, redirect completed onboarding to management, expose safe detail/edit views and type-aware Hebrew copy, retaining the existing server-derived ownership and financial validation.

The repository contains no fixed twelve-step goal sequence in either storage or goal UI. Goal milestones are immutable 25/50/75/100-percent historical crossings; sustained-success periods, versioned definitions, regressions and reopen events are not user-checkable tasks. Do not reinterpret these as an editable checklist. Retain financial-version changes and source correction + explicit reevaluation as the controlled route to current regression/reopening, preserving historical achievement. A new add/remove/skip task-plan feature needs a separate defined product contract; no such persistence model is introduced in this cleanup.

All seven legacy manual goal types retain nonnegative starting/current amounts and a positive target amount. Debt/overdraft amounts are explained as positive reported magnitudes, not signed canonical account balances; actual account balances still permit negatives. AI and manual values gain no verified authority. Saved goal title/priority editing cannot change material fields; transaction categories use existing correction evidence; provider records cannot be edited through manual forms. No authentication, schema, engine, provider, or financial persistence policy is relaxed.

## ADR-067 — One authorized refresh means one provider mutation attempt

The owner authorizes exactly one additional real Financy refresh at the documented 20-credit cost, without disconnect, and prohibits additional paid refreshes without fresh approval. Before dispatch, inspection found generic adapter retries also applied to mutations. Restrict automatic network/transient/401 retries to GET reads. Paid refresh and deletion each make at most one provider mutation attempt; an ambiguous outcome cannot authorize another. A failed paid lifecycle command cannot be replayed under the same key; preserve its receipt and require a separately authorized new command. This implements the existing explicit-cost/idempotency boundary, not a changed product policy. Token acquisition before the attempt remains server-only and carries no refresh charge. No paid request was made before this protection and its tests.

**Verified 2026-09-07:** 18 focused adapter/real-Mongo tests pass, including no POST retry on transport/401/503, bounded GET recovery, failed-key rejection and unchanged failure history. Exactly one real authenticated refresh was then sent at 04:25:57.028 UTC and recorded `completed/accepted` at 04:25:59.621 UTC. Provider data date advanced to 2026-09-07; both ordinary syncs completed and the second had identical financial/observation digests and zero duplicate groups. Full regression passed 74 files / 340 tests with real providers. Phase 9 is accepted within the existing staged boundary and ADR-066 exception; this does not claim real old-ID continuity or general hosted production onboarding. No new disconnect or billing/payment/plan-changing call occurred. The 20-credit value is the documented cost, not a separately inspected billing debit.

### Execution authority and next gate

The newest owner instruction supersedes earlier no-Phase-18 stops only after Phase 9 is verified, committed, pushed and clean. Phase 18 may then proceed subject to its original prerequisites; the roadmap is unchanged. The target remains Vercel with secured MongoDB, but the repository has only local loopback infrastructure, no linked Vercel project, and no verified hosted production configuration. SLO/backup/restore targets and audit-retention versus full erasure are unresolved operational/privacy choices. Do not invent these choices or claim production verification. No Phase 18 code is part of this Phase 9 commit.

## ADR-066 — Owner-approved development baseline archive, not production reconciliation

The 2026-09-06 owner instruction supersedes the manual-confirmation gate only for obsolete Financy test ingestion. The current ACTIVE connection becomes the new baseline after exact source/owner inventory, cross-reference rejection, BSON-preserving verified archival, and scoped retirement while local writers are stopped. Every other document is protected by pre/post digests; mixed/manual provenance or external references block the operation. Archive and manifest are server-only owner-scoped recovery evidence, not canonical financial inputs. Retain the binding and historical lifecycle/sync receipts. No wholesale collection deletion, remote provider mutation, or public reset endpoint.

Read-only real inventory identified 5 provider accounts, 460 provider transactions, 1 old connection and 1,187 old observation revisions. No non-target references or mixed canonical audit entries were found. Two manual accounts, two manual transactions, all other owners and all budgets/goals/forecasts/corrections/reviews/household/AI records remain protected. Retirement/import are not yet verified.

ADR-063/064/065 remain production policy and implemented safety mechanisms. Development replacement does not claim old-to-new identity continuity. Preserve separate bank products, minimized stable identity and real repeat-sync idempotency. No paid refresh without new approval; no Phase 18.

### ADR-066 verified cutover and remaining gate

The real cutover archived/retired exactly 1,653 proven obsolete documents and preserved all 101 protected documents without change, then imported 8 current products and 1,139 transaction observations. Repeat-sync canonical/observation digests matched; the authenticated production sync also changed zero canonical records. Completed owner-scoped migration aliases are excluded from subsequent ingestion, never generalized into a bypass for new production reconnects. Archive/manifest indexes are owner-first and unique; mixed provenance, references, concurrent changes, missing/damaged recovery copies and conflicting operator locks fail closed. An empty reconciliation review now reports `not_required` rather than claiming a nonexistent transaction block.

Final regression passed 74 files / 335 tests with real provider gates. Phase 9 remains unaccepted: historical refresh was provider-accepted but its application receipt failed schema validation. Preserve that evidence; a real success receipt on the corrected path is still required, and another documented 20-credit refresh needs explicit new owner approval. Disconnect is already verified and must not be repeated. No commit/push or Phase 18 at this checkpoint.
...
## ADR-049 — Net worth uses explicit valuation evidence and canonical aggregation

- **Decision:** Calculate current net worth as exact included assets minus exact included liabilities, grouped by currency, from actor-owned accounts, savings, loans, cards, and Phase 14 `netWorthItems`. Every component carries valuation type, effective time, provenance, freshness, source revision, and inclusion/exclusion evidence. Negative account balances become overdraft liabilities. Existing detailed savings take explicit precedence over savings-account fallback. Linked holdings declare parent-account or holding authority. Goal references never add asset value. Linked liability evidence uses fresh settlement, then fresh outstanding, then fresh deterministic-derived, then user-entered/principal priority; all-stale evidence remains visibly stale instead of being invented or dropped. No implicit FX is implemented.
- **Reasoning:** A net-worth number is meaningful only when each economic value has one aggregation authority and its valuation quality can be explained. Source labels or display names cannot reliably deduplicate accounts, holdings, savings, goals, cards, and debts.
- **Alternatives:** Summing every positive balance, adding holdings to aggregate investment accounts, counting savings goals as assets, treating APR as payoff, hiding stale evidence, and converting currencies using guessed rates were rejected.
- **Consequences:** Phase 14 introduces a pure versioned engine and explicit current-item relationship schema. Unrealized market/appraisal values may count but remain non-cash and cannot affect confirmed income, budgets, Safe to Spend, or verified goal progress. The manual/provider-neutral product works without claiming live market, brokerage, banking, or FX data.

## ADR-050 — Freshness is source/type-specific and net-worth history is immutable

- **Decision:** Version freshness as `net-worth-freshness-v1`. Source thresholds are 1 day (`verified_provider`, `market_data_provider`), 7 days (`deterministic_derived`), 14 days (`imported`), and 30 days (`user_entered`). Valuation-type thresholds are 1 day (`market_value`), 7 days (`cash_balance`, `account_balance`, `settlement_balance`, `outstanding_balance`, `derived_balance`), 30 days (`principal_balance`, `user_estimate`), and 90 days (`appraisal`); use the smaller threshold in the profile timezone. Persist immutable `netWorthSnapshots` with complete included/excluded component evidence and exact currency-grouped totals. Deduplicate by actor/date/state/policy fingerprint. Page reads never write; automatic material-change history is at most daily, while explicit materially changed snapshots may coexist.
- **Reasoning:** One hidden universal age limit misrepresents materially different evidence. Referencing mutable records alone cannot explain a historical statement after correction or deletion. Stable fingerprints provide retry safety without mutating evidence.
- **Alternatives:** One mutable current history row, snapshots on page load, unlimited automatic snapshots, reconstructing history from current documents, and later FX/valuation rewrites were rejected.
- **Consequences:** Corrections and soft deletion update current truth but cannot rewrite accepted history. Snapshot evidence remains owner-scoped, exact BSON int64, bounded in public views/exports, and separate from scenarios, goals, Safe to Spend, and AI. Full-account privacy erasure remains a separate legal/privacy boundary.

## Phase 14 verification addendum — 2026-09-02

- The versioned engine, canonical source mapper, audited current items, immutable snapshots, owner-scoped APIs/export, and protected Hebrew/RTL net-worth center implement ADR-049/050 without introducing provider, FX, payment, or AI calculation authority.
- Dedicated Phase 14 coverage passed 3 files / 19 tests. The complete final regression passed 53 files / 245 tests with real MongoDB and the real Anthropic gate; no real-provider or persistence acceptance was mocked or skipped.
- Production-browser acceptance with a real Auth.js database session added and reloaded one synthetic unrealized-market asset, kept it outside cash, generated one daily material-change snapshot, deduplicated an explicit same-state capture, rendered immutable history in Hebrew/RTL, and produced no browser warning/error logs.
...
- Financy is selected and the staged hosted-consent/in-memory-token boundary is verified in Phase 9. General multi-user programmatic onboarding, hosted production access and any future persisted-token/KMS design remain separate production gates; provider selection is no longer an open question.
...
- Phase 17 product/ethics policy is resolved by ADR-057 through ADR-059 and objectively verified. Phase 9's staged provider gate is now accepted. Phase 18 remains gated by concrete production infrastructure/access and its unresolved operational/privacy choices; no production-readiness claim follows from local provider acceptance.
...
## ADR-060 — Financy is the first Open Banking adapter and Auth.js ownership remains authoritative

- **Decision:** Implement Phase 9 against Financy / Open-Finance.ai behind a server-only provider-neutral port. The documented hosted Financy flow establishes provider consent; the Financial OS actor must explicitly claim the configured provider subject through a unique owner-scoped binding before any read or sync. Provider user IDs and external IDs are transport/provenance references, never authorization proof. Provider access tokens are short-lived, cached only in process memory, renewed once after `401`, and never persisted. Payment endpoints are out of scope.
- **Reasoning:** The real paid provider account supplies one end-user-scoped credential boundary, while Financial OS must retain independent per-user isolation. Short-lived tokens do not justify introducing unresolved persistent encryption/KMS claims. An explicit unique binding prevents two application actors from consuming the same configured provider subject.
- **Alternatives:** Trusting a client-supplied provider user ID, treating a provider ID as an owner, exposing tokens to the browser, storing short-lived bearer tokens in MongoDB, or invoking payment capabilities were rejected.
- **Consequences:** This local/staged configuration supports one provider subject per deployment. General multi-user production onboarding requires Financy's separate programmatic/B2B connection boundary or an equivalent provider credential-mapping facility before production launch; this limitation does not weaken present actor isolation.

## ADR-061 — Provider observations are exact immutable evidence before canonical financial truth

- **Decision:** Parse provider JSON number lexemes without using JavaScript floating point for financial values, convert decimal amounts to exact integer minor units, and preserve ISO currency. HMAC-alias stable provider references before persistence and discard raw identity, account-party, credential, and authorization payload fields. Append immutable versioned observation evidence for connections, accounts/balances, and transactions. Only literal `BOOKED` transactions become confirmed provider-neutral canonical transactions; `PENDING` and unknown statuses remain evidence. Unchanged fingerprints are idempotent, while changed observations append revisions. Manual records can never be overwritten by provider synchronization.
- **Reasoning:** The real API returns money as both JSON strings and numbers and exposes stable `SK`/ID values plus sensitive fields. Exact lexeme handling, stable aliases, and immutable revisions preserve correctness, pending/modified history, auditability, and provider substitution without retaining dangerous raw payloads.
- **Alternatives:** Floating-point parsing, raw-response storage, provider-specific canonical schemas, replacing pending rows in place without evidence, deleting records missing from a later pull, and heuristic confirmation were rejected.
- **Consequences:** Canonical provider transactions use the same owner-scoped `transactions` collection and deterministic engines as manual data, but remain read-only through manual mutation endpoints. Connection, account, balance, transaction revision, and sync-run evidence retain only minimized reproducibility metadata and exact BSON int64 amounts.

## ADR-062 — Phase 9 uses explicit bounded pull synchronization with costed lifecycle commands

- **Decision:** Synchronization is an explicit owner-scoped command with a lease, bounded cursor pagination, deterministic idempotency, durable partial/failure status, categorized retry policy, and recovery. Ordinary sync reads existing provider data and does not trigger the provider's paid refresh. The documented asynchronous refresh endpoint requires exact user confirmation of its current 20-credit cost; disconnect/delete requires a separate exact destructive confirmation. Financy currently documents no usable financial-data webhook authentication/signature/replay contract, so no data webhook is implemented or claimed.
- **Reasoning:** A pull-only lifecycle is the only behavior fully supported by current official documentation without inventing webhook security or silently spending credits. Durable run evidence is necessary to distinguish a complete snapshot from partial pagination/provider failure.
- **Alternatives:** Refresh on page load, hidden provider charges, automatic disconnect, unbounded pagination, invented webhook verification, retries of permanent errors, and treating missing pages as deletion were rejected.
- **Consequences:** `401` causes one token renewal/retry; transient provider/server failures use bounded backoff; invalid requests and authorization/consent failures fail closed. Connection expiry/freshness remain visible and separate from financial classification. Passive reads never mutate provider or canonical state.

### ADR-062 acceptance checkpoint — 2026-09-06

- The real user-approved refresh and destructive disconnect are prior lifecycle operations, not authorization to repeat them during reconnection diagnosis. The refresh response contract was corrected to official `accepted` after the original application receipt failed schema validation. An advancing `lastFetchedAt` records a completed fetch attempt, not independently successful new data or an audited credit debit; retain that distinction when completing lifecycle acceptance.
- The user's hosted reconnection confirmation was followed by real, read-only API verification using fresh token acquisition. Only `TERMINATED_BY_USER` and empty account/transaction pages were returned, with no omitted connection page. The provider access gate therefore remains failed. Do not infer another provider subject, loosen the existing owner binding, delete retained evidence, or import an empty replacement baseline.
- Preserve the current uncommitted implementation. Resume only when the same configured provider account exposes a readable real connection, then verify identity continuity and reconciliation before final acceptance. No Phase 9 acceptance/commit/push or Phase 18 implementation is authorized by this failed check.

## ADR-063 — Reconnection must fail closed when retained financial identities cannot be reconciled

- **Evidence (2026-09-06):** The unchanged private credentials now authenticate and expose both the new `ACTIVE` and old `TERMINATED_BY_USER` connections. Both explicitly match the configured provider subject; the new connection shares the historical institution alias. None of the current eight provider account aliases or 1,139 transaction aliases matches the five retained canonical account aliases or 460 canonical transaction aliases. Documented reads filtered to the old connection, including duplicates, return no records. Paid API access is verified; the exact Starter tier is user-reported, not independently attested by these data endpoints.
- **Decision under existing authority:** Add a server-side fail-closed preflight before any sync-run, observation, or canonical write. A readable connection cannot be imported while another ended connection for that actor and institution still has canonical financial records. Return a safe Hebrew reconciliation-required error; preserve all old financial records and immutable evidence. This enforces existing ownership, auditability, and no-duplicate-truth requirements; it does not approve a merge policy or weaken the failed acceptance gate.
- **Unresolved gate:** The previous normalizer retained provider-ID HMAC aliases but no durable bank-level identity that can bridge newly issued provider IDs. Automatic equality based on names, balances, dates, or amounts is insufficient. Historical account/transaction mapping needs authoritative provider evidence or an explicitly approved, auditable owner-review/cutover policy. Neither silent deduplication nor replacing old financial truth is authorized. Phase 9 remains unaccepted; Phase 18 remains prohibited.

## ADR-064 — Approved canonical reconnect identity and immutable reconciliation evidence

### Implementation authority update: explicit manual workflow

The user subsequently authorizes implementation of authenticated manual comparison/confirmation for this legacy-data limitation. ADR-065 below defines its implementation boundary; the preceding diagnostic stop is historical, not permission to infer account matches.

- **Authority (2026-09-06):** The user approves the durable `financy-reconnection-v1` policy recorded in both Master Plan copies. This resolves ADR-063's product-policy question, not the actual data-sufficiency or per-record identity gate.
- **Decision:** External IDs are historical aliases, not canonical identity. Automatic account mapping must have sufficient actual provider evidence and exactly one deterministic, non-conflicting unique-best match. Preserve canonical IDs and append external-alias lineage plus immutable matching fields, version, owner/actor or system, time, result class, and ambiguity evidence. Transactions depend on safe account mapping, guaranteed stable identifiers or strong unique fingerprints; amount/date-only matching and inferred pending/posted merging are forbidden.
- **Manual boundary:** Insufficient or conflicting evidence requires explicit authenticated owner review with safe comparisons and immutable decisions. An approval of this workflow is not an approval of guessed mappings. Preserve unresolved observations and historical canonical truth; do not turn ambiguity into duplicate balances or transactions. Minimize/hash sensitive identifiers; credentials/tokens remain server-only.
- **Reasoning and alternatives:** Provider reconnect can change transport IDs. Treating all new IDs as new money, deleting legacy history, fuzzy guesses, arbitrary tie-breaking, or silently rewriting provenance would violate financial integrity and auditability.
- **Consequences:** Retain ADR-063's pre-write guard until account and transaction safety is actually verified. First inventory real current fields and retained historical evidence through read-only diagnostics. If insufficient, stop and report missing evidence/manual review need as explicitly requested. Do not implement a guessed matching algorithm, call paid refresh, or claim final acceptance. The full original Phase 9 gate still applies; no Phase 18.

### ADR-064 real evidence inventory (2026-09-06)

The real read-only inventory verified current `accountNumber` presence on all eight provider accounts and parsed bank/branch/number on two. All eight historical account identities, including five canonical accounts, lack those retained fields. No historical stable-bank-reference/hash identity was modeled. Five current records have one institution/type/currency/minimized-name candidate each; that weak uniqueness is not an automatic identity decision. Current number presence cannot reconstruct discarded old identifiers. The guard rejected canonical sync and compared owner financial/provider checkpoints remained unchanged. The approved insufficient-evidence stop therefore applies: authoritative bridging evidence or explicit auditable manual reconciliation is required. This diagnostic neither implements manual reconciliation nor accepts Phase 9. Transaction matching is blocked by the parent-account gate; future acceptance tests must prove canonical continuity/alias history, not insist provider IDs never change.

## ADR-065 — Manual account identity ledger, explicit attestation, and a separate transaction gate

- **Authority:** The user approves the authenticated manual path for legacy accounts, including not-the-same/cannot-determine, safe real-provider comparisons, preserved history, and a mandatory stop for owner UI confirmation.
- **Decision:** Keep canonical financial documents unchanged by account confirmation. Attach external identities through `bankAccountReconciliations`, one owner/provider/canonical-account aggregate with a unique multi-key alias index, a single active identity, optimistic version, and append-only decision evidence. Each atomic update preserves prior aliases/events and captures server-derived actor, timestamp, policy, minimized old/new comparison evidence, result class, and client-request idempotency. Rejected/undetermined decisions do not claim aliases or resolve the account. This single-document atomicity plus unique indexes avoids partial multi-document financial remaps and does not assume a replica-set transaction facility.
- **Security:** Live candidates are read only after binding/actor validation. Server-derived opaque keys and a keyed review fingerprint bind each decision to the owner, canonical revision, ledger revision, and live candidate evidence; stale, foreign, duplicate, and conflicting submissions fail closed. No preselected candidate or automatic institution/name/type/currency match. Browser output contains only safe comparisons, not raw provider IDs or stable-reference hashes. Unknown/missing historical identity stays explicit.
- **Identity minimization:** The Financy adapter reduces documented account-number/parsed-account attributes to masked last digits, bounded public bank/branch codes, and a versioned domain-separated keyed digest before returning normalized observations. Future ingestion preserves these attributes in immutable observations. This prepares evidence, not an automatic reconciliation decision or stability guarantee.
- **Consequence:** Confirmation establishes an auditable account mapping but never starts transaction import, changes balances, or deduplicates history. Keep the existing sync safety gate until transaction continuity is separately implemented and verified under ADR-064. Stop at the ready real authenticated Hebrew/RTL UI for user-only confirmations; no acceptance, commit/push, paid provider action, or Phase 18 at that checkpoint.

### ADR-065 verified implementation checkpoint (2026-09-06)

The manual UI/service/API/atomic ledger and `financy-normalization-v2` minimized identity observations are implemented. Focused verification passed 23 tests, including the new real-Mongo rejection/uncertainty/immutable evidence/idempotency/ownership/race cases. The local non-external-provider regression passed 68 files / 319 tests; separate real read-only Financy/Mongo verification found five unresolved legacy accounts with safe masked candidates and unchanged financial/provider/ledger checkpoints. Type-check, zero-warning lint, production build, zero-vulnerability audit, and authenticated Hebrew/RTL production comparison checks passed. No real match was selected or confirmed by the agent. The ledger records owner attestation, not automatic identity certainty; live confirmations and subsequent deterministic transaction reconciliation remain required before Phase 9 acceptance. No Phase 18.
