...
import { prepareHouseholdInvitationQuarantine } from "@/lib/operations/household-invitation-closure";
...
(uri ? describe : describe.skip)("real isolated shared erasure transform", () => {
  it("restores survivor sharing metadata without deleted-member audit or finance corruption", async () => {
    ...
    try {
      ...
      await source.database.collection("householdInvitations").insertOne({ _id: invitationId, householdId: household,
        invitedByUserId: owner, acceptedByUserId: member, status: "accepted", createdAt: new Date(0), updatedAt: new Date(0),
        expiresAt: new Date(2000), version: 1, schemaVersion: 1, invitationPolicyVersion: "household-invitation-v1", auditTrail: [] });
      await source.database.collection("householdMemberships").insertOne({ ...common, _id: new ObjectId(), householdId: household,
        userId: member, activatedByInvitationId: invitationId, displayNameSnapshot: "Synthetic member", endedAt: null,
      ...
      const result = prepareHouseholdInvitationQuarantine(graph, await source.database.collection("householdInvitations").find().toArray(),
        { receipts: [receipt], environment: "isolated-test", keys: [key],
      ...
      expect(await target.database.collection("householdInvitations").countDocuments()).toBe(0);
      expect(result.evidence.freeTextReviewRequired).toBe(true);
