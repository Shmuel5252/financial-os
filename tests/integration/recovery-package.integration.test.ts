...
import { inspectManifestRecoverySources } from "@/lib/operations/manifest-source-recovery";
...
realMongo("reviewed application profile/manual schemas in isolated recovery package", () => {
  it("roundtrips real repository BSON through signed manifest, suppresses erased owner and recreates actual indexes", async () => {
    ...
    try {
      ...
      const snapshots = financialSnapshotRepositoryForDatabase(source.database); await snapshots.ensureIndexes();
      ...
      for (const actor of actors) {
        ...
        const rawAccount = (await source.database.collection("accounts").findOne({ userId: new ObjectId(actor.userId) }))!;
        await snapshots.createForActor(actor, "ILS", [{ section: "accounts", records: [{ id: rawAccount._id.toHexString(), version: rawAccount.version, updatedAt: rawAccount.updatedAt }] }], randomUUID());
      }
      ...
      expect(inspectManifestRecoverySources(opened.financialSnapshots!, { accounts: opened.accounts! }))
        .toEqual({ policy: "manifest-recovery-sources-v1", releaseAllowed: false, matched: 2, unresolved: { missing: 0, changed: 0, inactive: 0 } });
      ...
      for (const name of ["authUsers", "profiles", "accounts", "financialSnapshots"]) {
      ...
      expect(inspectManifestRecoverySources(await target.database.collection("financialSnapshots").find().toArray(), { accounts: [raw] }))
        .toEqual({ policy: "manifest-recovery-sources-v1", releaseAllowed: false, matched: 1, unresolved: { missing: 0, changed: 0, inactive: 0 } });
